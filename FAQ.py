...
description='''
- **POS**: Proof Of Stake. A consensus protocol used in cryptocurrencies that allows the creation of blocks through the balance owned by individuals and the integration of annual interest.
...
- **Superblock**: A superblock is a block generated by the network roughly every 24-28 hours containing the magnitude values of all active beacons (and associated CPIDs) in the Neural Network. These magnitude values are used to calculate the POR each user is owed.
...
- **Whitelist**: The set of projects that the Gridcoin network considers eligible for rewards. In order to receive GRC, you must be crunching for one of these projects. The chosen projects are determined by network voting.
